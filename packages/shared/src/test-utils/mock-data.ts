...
export const mockUserProfileData = (): UserProfileData => ({
  ...
  preferredSubjects: ['physics', 'chemistry', 'mathematics'].slice(
    0,
    Math.floor(Math.random() * 3) + 1
  ),
  studyGoals: ['improve_accuracy', 'time_management', 'concept_clarity'].slice(
    0,
    Math.floor(Math.random() * 3) + 1
  ),
...
export const mockTestSession = (
  ...
  return {
    ...
    status: isCompleted
      ? 'completed'
      : (['active', 'abandoned'][Math.floor(Math.random() * 2)] as any),
    startedAt,
    completedAt: isCompleted
      ? new Date(
          startedAt.getTime() + Math.floor(Math.random() * 4 * 60 * 60 * 1000)
        )
      : undefined,
    durationSeconds: isCompleted
      ? Math.floor(Math.random() * 3 * 60 * 60)
      : undefined,
    totalQuestions: 120,
...
export const mockQuestionMetadata = (
  ...
  conceptTags: ['basic', 'intermediate', 'advanced'].slice(
    0,
    Math.floor(Math.random() * 3) + 1
  ),
  ...overrides,
...
export const mockPerformanceAnalytics = (
  overrides: Partial<PerformanceAnalytics> = {}
): PerformanceAnalytics => {
  ...
  return {
    ...
    weaknesses: ['time_management', 'accuracy', 'concept_application'].slice(
      0,
      Math.floor(Math.random() * 3) + 1
    ),
