...
export function generateQuestions(params: GenerationParams): Question[] {
  ...
  for (let i = 0; i < params.count; i++) {
    const question = generateSingleQuestion(params.subject, params.difficulty, params.topic);
      ...
      easy: [
        {
          ...
          questionText: 'A body of mass 5 kg is moving with a velocity of 10 m/s. What is its kinetic energy?',
      ...
      medium: [
        {
          ...
          questionText: 'Two point charges of +3μC and -3μC are separated by 10 cm. What is the electric field at the midpoint?',
          options: ['5.4 × 10⁶ N/C', '1.08 × 10⁷ N/C', '2.7 × 10⁶ N/C', '0 N/C'],
      ...
      hard: [
        {
          ...
          questionText: 'Calculate the de Broglie wavelength of an electron moving with velocity 10⁶ m/s (mass = 9.1 × 10⁻³¹ kg, h = 6.63 × 10⁻³⁴ Js)',
          options: ['7.28 × 10⁻¹⁰ m', '7.28 × 10⁻¹¹ m', '7.28 × 10⁻⁹ m', '7.28 × 10⁻¹² m'],
          correctAnswer: '7.28 × 10⁻¹⁰ m',
          explanation: 'λ = h/mv = (6.63 × 10⁻³⁴)/(9.1 × 10⁻³¹ × 10⁶) = 7.28 × 10⁻¹⁰ m',
          conceptTags: ['de-broglie', 'quantum-mechanics', 'wave-particle-duality'],
        },
      ],
    },
    chemistry: {
      easy: [
        {
          ...
          questionText: 'What is the electronic configuration of Sodium (Na, atomic number 11)?',
          options: ['1s² 2s² 2p⁶ 3s¹', '1s² 2s² 2p⁶ 3s²', '1s² 2s² 2p⁵ 3s²', '1s² 2s² 2p⁶ 3p¹'],
          correctAnswer: '1s² 2s² 2p⁶ 3s¹',
          explanation: 'Sodium has 11 electrons distributed as: 2 in 1s, 2 in 2s, 6 in 2p, and 1 in 3s',
          conceptTags: ['electronic-configuration', 'atomic-structure', 'periodic-table'],
        },
      ],
      medium: [
        {
          ...
          questionText: 'What is the shape of SF₄ molecule according to VSEPR theory?',
          options: ['Tetrahedral', 'See-saw', 'Square planar', 'Trigonal bipyramidal'],
          correctAnswer: 'See-saw',
          explanation: 'SF₄ has 4 bonding pairs and 1 lone pair, resulting in see-saw geometry',
          conceptTags: ['vsepr-theory', 'molecular-geometry', 'chemical-bonding'],
        },
      ],
      hard: [
        {
          ...
          subtopic: 'Rate Laws',
          questionText: 'For a reaction A + B → C, doubling [A] doubles the rate, and doubling [B] quadruples the rate. What is the rate law?',
          options: ['Rate = k[A][B]', 'Rate = k[A][B]²', 'Rate = k[A]²[B]', 'Rate = k[A]²[B]²'],
          correctAnswer: 'Rate = k[A][B]²',
          explanation: 'Order with respect to A is 1 (rate doubles), order with respect to B is 2 (rate quadruples)',
      ...
      medium: [
        {
          ...
          options: ['3x² + 6x - 5', '3x² + 6x + 5', 'x² + 6x - 5', '3x² - 6x - 5'],
      ...
      hard: [
        {
          ...
          correctAnswer: '-π/6',
          explanation: 'sin(7π/6) = -1/2, and sin⁻¹(-1/2) = -π/6 (principal value)',
          conceptTags: ['inverse-trigonometry', 'trigonometry', 'principal-values'],
        },
      ],
    },
  };
...
export function validateQuestion(question: Question): ValidationResult {
  ...
  // Validate metadata
  if (question.metadata.estimatedTime < 30 || question.metadata.estimatedTime > 300) {
...
export async function storeQuestion(db: D1Database, question: Question): Promise<void> {
...
export function assessDifficulty(question: Question): 'easy' | 'medium' | 'hard' {
  let score = 0;
...
export function classifySubject(questionText: string): 'physics' | 'chemistry' | 'mathematics' | null {
  const text = questionText.toLowerCase();
  
  // Physics keywords
  const physicsKeywords = ['force', 'velocity', 'acceleration', 'energy', 'momentum', 'electric', 'magnetic', 'wave', 'quantum', 'mass', 'newton', 'joule'];
  const physicsCount = physicsKeywords.filter(kw => text.includes(kw)).length;
  
  // Chemistry keywords
  const chemistryKeywords = ['atom', 'molecule', 'reaction', 'bond', 'electron', 'element', 'compound', 'acid', 'base', 'oxidation', 'reduction', 'mole'];
  const chemistryCount = chemistryKeywords.filter(kw => text.includes(kw)).length;
  
  // Mathematics keywords
  const mathKeywords = ['equation', 'solve', 'derivative', 'integral', 'matrix', 'vector', 'function', 'polynomial', 'trigonometry', 'logarithm'];
...
export function extractTopicTags(question: Question): string[] {
  ...
  if (question.subject === 'physics') {
    if (text.includes('force') || text.includes('newton')) tags.add('mechanics');
    if (text.includes('electric') || text.includes('charge')) tags.add('electromagnetism');
    if (text.includes('wave') || text.includes('frequency')) tags.add('waves');
    if (text.includes('quantum') || text.includes('photon')) tags.add('modern-physics');
    if (text.includes('heat') || text.includes('temperature')) tags.add('thermodynamics');
  } else if (question.subject === 'chemistry') {
    if (text.includes('atom') || text.includes('electron')) tags.add('atomic-structure');
    if (text.includes('bond') || text.includes('molecule')) tags.add('chemical-bonding');
    if (text.includes('reaction') || text.includes('rate')) tags.add('chemical-kinetics');
    if (text.includes('acid') || text.includes('base')) tags.add('acids-bases');
    if (text.includes('organic') || text.includes('carbon')) tags.add('organic-chemistry');
  } else if (question.subject === 'mathematics') {
    if (text.includes('derivative') || text.includes('differentiation')) tags.add('calculus');
    if (text.includes('integral') || text.includes('integration')) tags.add('calculus');
    if (text.includes('matrix') || text.includes('determinant')) tags.add('linear-algebra');
    if (text.includes('sin') || text.includes('cos') || text.includes('tan')) tags.add('trigonometry');
    if (text.includes('equation') || text.includes('solve')) tags.add('algebra');
  }
...
export function validateQuestionDistribution(
  ...
  if (questions.length !== requirements.totalQuestions) {
    errors.push(`Expected ${requirements.totalQuestions} questions, got ${questions.length}`);
  }
  ...
  Object.entries(requirements.subjectDistribution).forEach(([subject, expected]) => {
    const actual = subjectCounts[subject] || 0;
    const tolerance = Math.ceil(expected * 0.1); // 10% tolerance
    
    if (Math.abs(actual - expected) > tolerance) {
      errors.push(`Subject ${subject}: expected ${expected} questions (±${tolerance}), got ${actual}`);
    }
  });
  ...
  Object.entries(requirements.difficultyDistribution).forEach(([difficulty, expected]) => {
    const actual = difficultyCounts[difficulty] || 0;
    const tolerance = Math.ceil(expected * 0.15); // 15% tolerance for difficulty
    
    if (Math.abs(actual - expected) > tolerance) {
      warnings.push(`Difficulty ${difficulty}: expected ${expected} questions (±${tolerance}), got ${actual}`);
    }
  });
