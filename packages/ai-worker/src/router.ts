...
// Route handlers
export async function handleGenerateQuestions(request: Request, env: Env): Promise<Response> {
  const origin = request.headers.get('Origin');
  const corsOrigins = env.CORS_ORIGINS || '*';
  ...
  try {
    ...
    // Validate input
    if (!body.subject || !['physics', 'chemistry', 'mathematics'].includes(body.subject)) {
      return errorResponse('Invalid subject. Must be physics, chemistry, or mathematics', 400, corsHeaders);
    }
    
    if (!body.difficulty || !['easy', 'medium', 'hard'].includes(body.difficulty)) {
      return errorResponse('Invalid difficulty. Must be easy, medium, or hard', 400, corsHeaders);
    }
    
    const maxBatch = parseInt(env.MAX_QUESTIONS_PER_BATCH || '50');
    if (!body.count || body.count < 1 || body.count > maxBatch) {
      return errorResponse(`Invalid count. Must be between 1 and ${maxBatch}`, 400, corsHeaders);
    }
    ...
    return errorResponse(error.message || 'Failed to generate questions', 500, corsHeaders);
  }
}

export async function handleValidateQuestion(request: Request, env: Env): Promise<Response> {
  const origin = request.headers.get('Origin');
  const corsOrigins = env.CORS_ORIGINS || '*';
  const corsHeaders = getCorsHeaders(origin, corsOrigins);
  ...
  try {
    ...
    return errorResponse(error.message || 'Failed to validate question', 500, corsHeaders);
  }
}
...
export async function handleGetQuestions(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const questions = await getQuestions(env.DB, { subject, difficulty, limit });
    ...
    return errorResponse(error.message || 'Failed to retrieve questions', 500, corsHeaders);
  }
}

export async function handleGetQuestion(request: Request, env: Env, questionId: string): Promise<Response> {
  const origin = request.headers.get('Origin');
  const corsOrigins = env.CORS_ORIGINS || '*';
  const corsHeaders = getCorsHeaders(origin, corsOrigins);
  ...
  try {
    ...
    return errorResponse(error.message || 'Failed to retrieve question', 500, corsHeaders);
  }
}
...
export async function router(request: Request, env: Env): Promise<Response> {
  ...
  if (path === '/ai/health' && method === 'GET') {
    ...
    const corsOrigins = env.CORS_ORIGINS || '*';
    const corsHeaders = getCorsHeaders(origin, corsOrigins);
    return successResponse({ status: 'healthy', service: 'ai-worker' }, corsHeaders);
  }
  ...
  if (path === '/ai/assess-difficulty' && method === 'POST') {
    ...
    try {
      ...
      return errorResponse(error.message || 'Failed to assess difficulty', 500, corsHeaders);
    }
  }
  ...
  if (path === '/ai/classify-subject' && method === 'POST') {
    ...
    try {
      ...
      return errorResponse(error.message || 'Failed to classify subject', 500, corsHeaders);
    }
  }
  ...
  if (path === '/ai/extract-tags' && method === 'POST') {
    ...
    try {
      ...
      return errorResponse(error.message || 'Failed to extract tags', 500, corsHeaders);
    }
  }
  ...
  if (path === '/ai/validate-distribution' && method === 'POST') {
    ...
    try {
      const body = await request.json() as { questions: Question[]; requirements: DistributionRequirements };
      const validation = validateQuestionDistribution(body.questions, body.requirements);
      return successResponse(validation, corsHeaders);
    } catch (error: any) {
      return errorResponse(error.message || 'Failed to validate distribution', 500, corsHeaders);
    }
  }
