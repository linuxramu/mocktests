    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          topic: fc.option(fc.string({ minLength: 3, maxLength: 20 }), { nil: undefined }),
        }),
    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          explanation: fc.option(fc.string({ minLength: 10, maxLength: 200 }), { nil: undefined }),
          sourcePattern: fc.string({ minLength: 5, maxLength: 50 }),
          metadata: fc.record({
            topic: fc.string({ minLength: 3, maxLength: 50 }),
            subtopic: fc.option(fc.string({ minLength: 3, maxLength: 50 }), { nil: undefined }),
            yearSource: fc.option(fc.integer({ min: 2000, max: 2030 }), { nil: undefined }),
      ...
      fc.property(
        ...
        (config) => {
          ...
          for (let i = 0; i < config.physicsCount; i++) {
            const difficulty = i < config.easyCount ? 'easy' : i < config.easyCount + config.mediumCount ? 'medium' : 'hard';
            questions.push(...generateQuestions({ subject: 'physics', difficulty, count: 1 }));
          }
          
          // Generate chemistry questions
          for (let i = 0; i < config.chemistryCount; i++) {
            const difficulty = i < config.easyCount ? 'easy' : i < config.easyCount + config.mediumCount ? 'medium' : 'hard';
            questions.push(...generateQuestions({ subject: 'chemistry', difficulty, count: 1 }));
          }
          
          // Generate mathematics questions
          for (let i = 0; i < config.mathematicsCount; i++) {
            const difficulty = i < config.easyCount ? 'easy' : i < config.easyCount + config.mediumCount ? 'medium' : 'hard';
            questions.push(...generateQuestions({ subject: 'mathematics', difficulty, count: 1 }));
          }
          
          const totalQuestions = config.physicsCount + config.chemistryCount + config.mathematicsCount;
          ...
          if (questions.length === totalQuestions) {
            ...
            const totalError = validation.errors.find(e => e.includes('Expected'));
